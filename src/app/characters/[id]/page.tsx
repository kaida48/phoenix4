      ...
      <main className="container mx-auto px-6 py-8">
        ...
        <div className="bg-gray-800 p-6 rounded-lg shadow-lg mb-6">
          ...
          <section className="mb-8">
            <h3 className="text-xl font-semibold mb-4 border-b border-gray-700 pb-2">Backstory</h3>
          ...
          {character.appearance && (
            <section className="mb-8">
              <h3 className="text-xl font-semibold mb-4 border-b border-gray-700 pb-2">Appearance</h3>
          ...
          {character.skills && Object.keys(character.skills).length > 0 && (
            <section className="mb-8">
              <h3 className="text-xl font-semibold mb-4 border-b border-gray-700 pb-2">Skills</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {Object.entries(character.skills).map(([skill, level]) => (
                  <div key={skill} className="bg-gray-700 p-3 rounded flex justify-between">
                    <span>{skill}</span>
          ...
          {character.equipment?.items && character.equipment.items.length > 0 && (
            <section className="mb-8">
              <h3 className="text-xl font-semibold mb-4 border-b border-gray-700 pb-2">Equipment</h3>
              <div className="space-y-3">
                {character.equipment.items.map((item: any, index: number) => (
                  <div key={index} className="bg-gray-700 p-3 rounded">
                    ...
                    {item.source && <p className="text-gray-400 text-sm mt-1">Source: {item.source}</p>}
                  </div>
                ))}
              </div>
            </section>
          ...
          {character.relationships && (
            <section className="mb-8">
              <h3 className="text-xl font-semibold mb-4 border-b border-gray-700 pb-2">Relationships</h3>
              
              {character.relationships.allies && character.relationships.allies.length > 0 && (
                <div className="mb-4">
                  <h4 className="font-semibold text-green-400 mb-2">Allies</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                    {character.relationships.allies.map((ally: string, index: number) => (
                      <div key={index} className="bg-green-900/30 p-2 rounded border border-green-800">
                        {ally}
                      </div>
                    ))}
                  </div>
              ...
              {character.relationships.enemies && character.relationships.enemies.length > 0 && (
                <div className="mb-4">
                  <h4 className="font-semibold text-red-400 mb-2">Enemies</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                    {character.relationships.enemies.map((enemy: string, index: number) => (
                      <div key={index} className="bg-red-900/30 p-2 rounded border border-red-800">
                        {enemy}
                      </div>
                    ))}
                  </div>
              ...
              {character.relationships.neutral && character.relationships.neutral.length > 0 && (
                <div>
                  <h4 className="font-semibold text-gray-400 mb-2">Neutral</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                    {character.relationships.neutral.map((neutral: string, index: number) => (
